...
| G4 / M3 | `ready_for_human_validation` | User-scoped projection、applicability/freshness/conflict policy、planner/evaluator isolation與既有 5-case outcome eval 已完成；新 session 僅在 default-off observe flag 開啟時，才按「最後被回答的題」套用同-role replacement/revalidation，且不可影響 scoring。 | [M3 evidence](evidence/m3-memory-outcomes.md) | `LOCAL_MEMORY_OUTCOME_GATE_PASS_DEFAULT_OFF` |
