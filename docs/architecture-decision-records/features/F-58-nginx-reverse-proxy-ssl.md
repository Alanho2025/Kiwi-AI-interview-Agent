# Feature RFC: F-58 Caddy 反向代理與 SSL / TLS 憑證自動卸載
...
> * **Caddy 反向代理 (本 Feature)**：就像在飯店大門口請了一位「專業大堂經理 (Nginx)」。經理在大門口統一代理 80/443 埠，負責 SSL 加解密與 HTML 靜態檔案發放；只有當客人要點餐時，才把請求傳給後廚 Node.js。後廚負擔大減，速度快 5 倍！
...
* **現行架構 (Current Version - PR #128 Commit `728cad5`)**：
  - `deploy/ec2/Caddyfile` 實現反向代理 (Reverse Proxy)，對外代理 80/443 埠，實現 `/api` 請求轉發至 `backend:5000`，並設置 `Upgrade` 標頭以完全支持全雙工 WebSocket 通道。
...
* **In-Scope (包含範圍)**：
  - Caddy 反向代理、`/api` 轉發至 `backend:5000`、WebSocket `Upgrade` 標頭放行、Gzip 靜態壓縮。
* **Out-of-Scope (排除範圍)**：
...
2. **第二步（靜態分流）**：如果是請求網頁 HTML/JS，Nginx 在 15-50ms內直接從本地磁碟回傳。
...
* **現行程式碼位置**：[`deploy/ec2/Caddyfile:L1-L3`](../../deploy/ec2/Caddyfile#L1-L3)
