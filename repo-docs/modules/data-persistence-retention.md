...
当前 schema 有 `is_encrypted`、virus scan status、retention fields、audit logs 和 deletion requests，但不能把它们写成完整合规保证。是否启用 worker、如何部署存储、是否完成 account-wide deletion、是否有 encryption-at-rest enforcement，都需要独立验证。
