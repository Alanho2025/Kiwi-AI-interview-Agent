...
證據狀態：本頁基於 2026-07-11 項目源碼已確認；目前已支援 bounded same-origin website evidence capture、manual/website explicit conflict diagnostics、`company_understanding_v2` detail fields、deterministic `role_intent_decoder_v2` fields 與 diagnostics、`candidate_evidence_graph_v2`、`role_evidence_map_v2`、proof strategy preparation guidance、`answer_alignment_v2` first slice、`role_fit_diagnostics_v1` compact propagation、12-case mock-safe adversarial eval gate、12/12 human calibration 和 Role-Fit release gate。Release gate 目前為 `ready_with_known_issues`，唯一 known issue 是 real-backend voice next-question first audio 超過 3 秒；production corpus / real-generation semantic judge 與 production telemetry 仍需外部驗證。
