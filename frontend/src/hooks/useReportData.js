...
import { downloadSessionRecording, getSessionRecordingStatus, retryRecordingUpload } from '../api/recordingApi.js';
...
const CONTINUOUS_RECORDING_STATES = new Set([
  'receiving',
  'uploading',
  'locally_durable',
  'waiting_for_network',
...
const toRecordingStatusState = (result = {}) => ({
  ...result,
  state: result?.state || (result?.available ? 'ready' : 'missing'),
...
export function useReportData(sessionId) {
  ...
  const loadReport = useCallback(async ({ autoGenerateIfMissing = false } = {}) => {
    try {
      ...
      setReportData(data);
      const isLegacyReport = String(data?.report?.schemaVersion || '').toLowerCase() === 'v5';
      setStatus(isLegacyReport
        ? buildStatus('info', 'Legacy report loaded', 'Historical scores were preserved. Regenerate this report for corrected scoring and safe rewrite content.')
        : buildStatus('success', 'Report loaded', 'Your latest report and verification status are shown below.'));
      return data;
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const scheduleNextCheck = (callback, state) => {
      if (CONTINUOUS_RECORDING_STATES.has(state)) {
        timer = window.setTimeout(callback, RECORDING_STATUS_POLL_INTERVAL_MS);
        return;
      }
    ...
    const loadRecordingStatus = async () => {
      try {
        ...
        if (!nextStatus.available && nextStatus.retryable) {
          scheduleNextCheck(loadRecordingStatus, nextStatus.state);
        }
      } catch (error) {
        if (cancelled) return;
        setRecordingStatus({
  ...
  useEffect(() => {
    if (!sessionId || typeof indexedDB === 'undefined') return undefined;
    const manager = recordingUploadRegistry.getOrCreate(sessionId);
    const unsubscribe = manager.subscribe((nextStatus) => {
      if (nextStatus.state !== 'idle') setRecordingStatus(toRecordingStatusState(nextStatus));
    });
    void manager.start();
    return unsubscribe;
  }, [sessionId]);
  ...
  const handleRetryRecording = useCallback(async () => {
    if (!recordingStatus.uploadId || recordingStatus.retryable === false) return;
    try {
      const result = await retryRecordingUpload(recordingStatus.uploadId);
      setRecordingStatus(toRecordingStatusState(result));
    } catch (error) {
      setRecordingStatus((current) => ({ ...current, state: 'recoverable_failed', error }));
    }
  }, [recordingStatus.retryable, recordingStatus.uploadId]);
