...
export function useDuplexVoiceSocket({
  ...
  const connect = useCallback(({ sessionId, language = DEFAULT_LANGUAGE, sampleRate = DEFAULT_SAMPLE_RATE, voiceName = DEFAULT_VOICE_NAME }) => new Promise((resolve, reject) => {
    ...
    socket.onmessage = (event) => {
      ...
      if (payload.type === 'transcript_confirmation_requested') {
        console.info('[FRONTEND-WS-TRACE] transcript_confirmation_requested', {
          socketTraceSession: socketTraceSessionRef.current,
          speechTurnTrace: speechTurnTraceRef.current,
        ...
        callbacksRef.current.onTranscriptConfirmationRequested?.(payload);
        return;
      }
      if (payload.type === 'transcript_confirmation_resolved') {
        console.info('[FRONTEND-WS-TRACE] transcript_confirmation_resolved', {
          socketTraceSession: socketTraceSessionRef.current,
          speechTurnTrace: speechTurnTraceRef.current,
          decision: payload.decision,
          at: Date.now(),
        });
        callbacksRef.current.onTranscriptConfirmationResolved?.(payload);
        return;
      }
