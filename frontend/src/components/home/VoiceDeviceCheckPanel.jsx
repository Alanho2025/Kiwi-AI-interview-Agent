    ...
    <div className="rounded-xl border border-gray-200 bg-white px-4 py-3">
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-start gap-3">
          <span className="mt-0.5 rounded-full bg-gray-100 p-2 text-gray-600"><Icon className="h-4 w-4" /></span>
          <div>
        ...
        <div className={cn('flex shrink-0 items-center gap-1 rounded-full px-2.5 py-1 text-xs font-medium', isOk ? 'bg-green-50 text-green-700' : isChecking ? 'bg-blue-50 text-blue-700' : 'bg-gray-100 text-gray-600')}>
    ...
    <div className="rounded-2xl border border-gray-200 bg-gray-50 p-4">
      <div className="mb-4 flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-xs text-gray-500">Check browser support, microphone input, and speaker output before starting Voice Mode.</p>
          {checkedAtLabel ? <p className="mt-1 text-xs text-gray-400">Last checked: {checkedAtLabel}</p> : null}
        </div>
        <span className={cn('rounded-full px-3 py-1 text-xs font-medium', isMicReady ? 'bg-green-50 text-green-700' : 'bg-yellow-50 text-yellow-700')}>{statusLabel}</span>
      </div>
      ...
      <div className="mt-4 flex flex-wrap gap-3">
        <button type="button" className="rounded-full border border-emerald-200 bg-white px-4 py-2 text-sm font-semibold text-emerald-700 transition hover:bg-emerald-50 disabled:cursor-not-allowed disabled:opacity-60" onClick={checkMicrophone} disabled={deviceCheck.mic.status === 'checking'}>Check Microphone</button>
        <button type="button" className="rounded-full border border-emerald-200 bg-white px-4 py-2 text-sm font-semibold text-emerald-700 transition hover:bg-emerald-50 disabled:cursor-not-allowed disabled:opacity-60" onClick={checkSpeaker} disabled={deviceCheck.speaker.status === 'checking'}>Play Speaker Test</button>
        {deviceCheck.speaker.status === 'needs_confirmation' ? (
          <>
            <button type="button" className="rounded-full border border-emerald-200 bg-white px-4 py-2 text-sm font-semibold text-emerald-700 transition hover:bg-emerald-50" onClick={confirmSpeakerHeard}>I heard it</button>
            <button type="button" className="rounded-full border border-red-200 bg-white px-4 py-2 text-sm font-semibold text-red-700 transition hover:bg-red-50" onClick={confirmSpeakerNotHeard}>I did not hear it</button>
          </>
        ) : null}
      </div>
    </div>
