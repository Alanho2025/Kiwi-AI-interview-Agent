...
export function EndSessionProgress({ progress }) {
  ...
  return (
    <div className={cn('rounded-2xl border bg-white p-4 shadow-sm', isFailed ? 'border-red-200' : 'border-sky-100')}>
      <div className="flex items-center justify-between gap-3">
        <div>
          <p className="text-sm font-semibold text-gray-900">Ending interview</p>
          <p className="mt-1 text-xs text-gray-500">Please keep this page open while we save your session.</p>
        </div>
      ...
      <div className="mt-4 grid gap-2 sm:grid-cols-3">
        {steps.map((step, index) => {
          ...
          return (
            <div key={step.key} className="flex items-center gap-2 rounded-xl border border-gray-100 bg-gray-50 px-3 py-2 text-sm">
              <span className={cn(
                'flex h-6 w-6 items-center justify-center rounded-full text-xs font-bold',
                isDone ? 'bg-emerald-100 text-emerald-700' : isActive ? 'bg-sky-100 text-sky-700' : 'bg-white text-gray-400'
              )}>
                {isDone ? '✓' : index + 1}
              </span>
              <span className={cn(isActive ? 'font-semibold text-gray-900' : 'text-gray-500')}>
                {step.label}
              </span>
            </div>
          );
