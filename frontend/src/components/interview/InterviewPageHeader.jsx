...
export function InterviewPageHeader({ session, title, roleFamilyLabel, exactRoleTitle, modeLabel, levelLabel, stageLabel, elapsedSeconds, controlMode = 'question_limited', timeLimitSeconds = null, isVoiceMode = false, onViewReport }) {
  ...
  const buttonClass = hasReport
    ? 'rounded-full [background:var(--accent)] text-primary px-6 py-3 text-sm font-semibold hover:[background:var(--accent)] shadow-md hover:shadow-lg transition-all duration-200'
    : 'rounded-full [background:var(--accent)] text-primary px-6 py-3 text-sm font-semibold hover:[background:var(--accent)] shadow-md hover:shadow-lg transition-all duration-200';
      ...
      <AppHeader>
        <div className="flex flex-wrap items-center justify-between gap-3 sm:gap-6 min-w-0">
          <div className="flex items-center gap-3 min-w-0">
            <span className="hidden sm:inline text-sm text-faint">Target Role</span>
            <span className="px-3 py-1.5 sm:px-4 sm:py-2 [background:var(--accent-glow)] text-accent text-xs sm:text-sm font-medium rounded-full max-w-[180px] sm:max-w-[260px] truncate">
              {exactRoleTitle || roleFamilyLabel}
            </span>
          </div>
          <div className="hidden sm:block min-w-0 flex-1 text-center">
            <div className="text-lg sm:text-[28px] font-semibold text-primary truncate">{buildPrimaryTitle({ isVoiceMode, title })}</div>
          </div>
          <div className="flex items-center gap-4 sm:gap-8 shrink-0">
            <div className="text-right">
              <p className="text-xs text-faint uppercase tracking-wider">{timerLabel}</p>
              <p className="text-[20px] sm:text-[28px] font-mono font-medium text-primary">{timerValue}</p>
            </div>
            <div className="text-right">
              <p className="text-xs text-faint uppercase tracking-wider">Progress</p>
    ...
    <AppHeader>
      <div className="flex flex-wrap items-center justify-end gap-3 sm:gap-6 min-w-0">
        <div className="flex items-center gap-2 min-w-0">
          <span className="text-sm text-faint">Role family</span>
          <span className="px-3 py-1 [background:var(--accent-glow)] text-accent text-sm font-medium rounded-full max-w-[220px] truncate">
            {roleFamilyLabel}
          </span>
        </div>
        <div className="min-w-0 flex-1 max-w-[420px]">
          <div className="text-lg font-semibold text-primary truncate">{title}</div>
        ...
        <div className="flex items-center gap-6 shrink-0">
          <div className="text-right">
            <p className="text-xs text-faint uppercase tracking-wider">{timerLabel}</p>
            <p className="text-lg font-mono font-medium text-primary">{timerValue}</p>
          </div>
          <div className="text-right">
            <p className="text-xs text-faint uppercase tracking-wider">Progress</p>
            <p className="text-sm font-medium text-primary">{progressText}</p>
