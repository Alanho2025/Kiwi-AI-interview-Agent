...
describe('voiceActivityDetectionCore', () => {
  it('detects speech start and speech end after enough silence', () => {
    const vad = createVoiceActivityStateMachine({ warmupIgnoreMs: 0, minSpeechMs: 500, silenceToStopMs: 1000 });
    vad.start(0);
  ...
  it('does not submit very short speech', () => {
    const vad = createVoiceActivityStateMachine({ warmupIgnoreMs: 0, minSpeechMs: 700, silenceToStopMs: 1000 });
    vad.start(0);
    expect(vad.update(0.03, 100).event).toBe('speech_start');
