...
import { buildScenarioPolicyItem } from './questionPoolComposerService.js';
...
const buildScenarioOutput = (scenario) => {
  ...
  const scenarioPolicyItem = buildScenarioPolicyItem({
    analysisResult: context.analysisResult,
    context,
    settings: context.settings,
  });
  const reservationPlan = resolveCatalogReservationPlan({
    poolItems: [...snapshots.items, ...(scenarioPolicyItem ? [scenarioPolicyItem] : [])],
    session: {
      analysisResult: context.analysisResult,
      settings: context.settings,
...
export const buildVoiceSelectionPolicyReviewSnapshot = () => ({
  ...
  generatedFrom: [
    ...
    'questionPoolComposerService.js',
  ],
...
export const renderVoiceSelectionPolicyReviewMarkdown = ({ policyReviewRecord = {} } = {}) => {
  ...
  return [
    ...
    `Selection policy version: \`${policyReviewRecord.policyVersion || 'not_provided'}\``,
