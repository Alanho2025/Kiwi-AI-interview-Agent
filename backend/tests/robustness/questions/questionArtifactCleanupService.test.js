...
vi.mock('../../../src/db/models/documentChunkModel.js', () => ({
  DocumentChunk: { deleteMany: mocks.deleteDocumentChunks },
}));
...
describe('questionArtifactCleanupService', () => {
  beforeEach(() => {
    ...
    mocks.deleteDocumentChunks.mockResolvedValue({ deletedCount: 2 });
  ...
  it('deletes prepared pool, JD filter, and CV seeds after report generation', async () => {
    ...
    expect(mocks.deleteDocumentChunks).toHaveBeenCalledWith({ sessionId: 'session-1', sourceType: 'prepared_question_pool' });
  ...
  it('returns zero counts when required ownership identifiers are missing', async () => {
    ...
    expect(mocks.deleteDocumentChunks).not.toHaveBeenCalled();
